#!/usr/bin/env bash
# Batch 2: more conservative Stage 5->6 attempts after batch 1.
set -euo pipefail
...
STAGE3_CKPT="${REPO_ROOT}/results/Checkpoints/VLOG_VLA_LIBERO_STAGE3_V2/checkpoints/steps_12000_pytorch_model.pt"
...
# aid|s5_run|s5_steps|s6_run|s6_max|base_ckpt|lr|lr_router|grad|s5_save|s6_extra
ATTEMPTS=(
  "5|VLOG_VLA_LIBERO_STAGE5_RETRY5|12000|VLOG_VLA_LIBERO_FULL_RETRY5|25000|${STAGE4_CKPT}|3e-5|0.1|0.2|1000|--trainer.max_consecutive_nan_steps 30"
  "6|VLOG_VLA_LIBERO_STAGE5_RETRY6|10000|VLOG_VLA_LIBERO_FULL_RETRY6|20000|${STAGE4_CKPT}|5e-6|0.05|0.15|1000|--trainer.max_consecutive_nan_steps 20 --framework.vlog.losses.lambda_critic 0.1"
  "7|VLOG_VLA_LIBERO_STAGE5_RETRY7|15000|VLOG_VLA_LIBERO_FULL_RETRY7|30000|${STAGE3_CKPT}|2e-5|0.15|0.3|1000|--trainer.max_consecutive_nan_steps 50"
  "8|VLOG_VLA_LIBERO_STAGE5_RETRY8|8000|VLOG_VLA_LIBERO_FULL_RETRY8|15000|${STAGE4_CKPT}|1e-5|0.05|0.1|500|--trainer.max_consecutive_nan_steps 10 --framework.vlog.losses.lambda_critic 0.05 --framework.vlog.losses.lambda_router 0.05"
)
...
train_stage() {
  local run_id="$1" stage="$2" max_steps="$3" base_ckpt="$4" save_int="$5" lr="$6" lr_router="$7" grad="$8" extra="$9"
  echo "[$(date -Is)] TRAIN ${run_id} stage=${stage} steps=${max_steps} base=${base_ckpt}" | tee -a "${LOG_DIR}/loop.log"
  cd "${REPO_ROOT}"
  ...
  env "${COMMON[@]}" RUN_ID="${run_id}" TRAIN_STAGE="${stage}" \
    ...
    EXTRA_TRAIN_ARGS="${extra_args} --trainer.learning_rate.vlog ${lr} --trainer.learning_rate.action_model ${lr} --framework.vlog.losses.lambda_router ${lr_router} --trainer.gradient_clipping ${grad}" \
...
eval_and_score() {
  ...
  RUN_ID="${run_id}" OUTPUT_ROOT="${out}" MANIFEST="${out}/manifest.jsonl" CKPT="${ckpt}" \
    PARALLEL_JOBS=4 BASE_PORT=6200 \
...
for entry in "${ATTEMPTS[@]}"; do
  IFS='|' read -r aid s5_run s5_steps s6_run s6_max base_ckpt lr lr_router grad s5_save s6_extra <<< "${entry}"
  echo "======== BATCH2 ATTEMPT ${aid} ========" | tee -a "${LOG_DIR}/loop.log"

  S5_CKPT=$(train_stage "${s5_run}" "stage5_router" "${s5_steps}" "${base_ckpt}" "${s5_save}" "${lr}" "${lr_router}" "${grad}" "")
  S5_LOG="${REPO_ROOT}/results/Checkpoints/${s5_run}/logs/train_log.jsonl"
  if [[ -z "${S5_CKPT}" || ! -f "${S5_CKPT}" ]] || ! check_train_ok "${S5_LOG}" 500; then
    echo "[$(date -Is)] BATCH2 ATTEMPT ${aid} Stage5 FAILED" | tee -a "${LOG_DIR}/loop.log"
    continue
  fi

  S6_EXTRA="--trainer.learning_rate.vlog ${lr} --trainer.learning_rate.action_model ${lr} --framework.vlog.losses.lambda_router 0.1 --trainer.gradient_clipping ${grad} --trainer.early_stop_patience 2000 --trainer.early_stop_window 200 --trainer.early_stop_min_delta 0.001 --trainer.early_stop_min_steps 5000 --trainer.early_stop_metric vlog/action_loss ${s6_extra}"

  S6_CKPT=$(train_stage "${s6_run}" "stage6_full" "${s6_max}" "${S5_CKPT}" 2000 "${lr}" "0.1" "${grad}" "${S6_EXTRA}")
  S6_LOG="${REPO_ROOT}/results/Checkpoints/${s6_run}/logs/train_log.jsonl"
  if [[ -z "${S6_CKPT}" || ! -f "${S6_CKPT}" ]] || ! check_train_ok "${S6_LOG}" 300; then
    echo "[$(date -Is)] BATCH2 ATTEMPT ${aid} Stage6 FAILED" | tee -a "${LOG_DIR}/loop.log"
    continue
  fi
...
echo "[$(date -Is)] Batch2 done." | tee -a "${LOG_DIR}/loop.log"
